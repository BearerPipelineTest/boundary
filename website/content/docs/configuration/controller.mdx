...
- `ops_listeners_grace_period` - Amount of time Boundary will wait before initiating the shutdown procedure,
  ...
  used when an `ops` listener is set. Default is 0 seconds.
...
controller {
  ...
  ops_listeners_grace_period = 10s
